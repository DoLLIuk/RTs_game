...
public sealed partial class GameSimulation
{
	...
	private HarassObjective SelectHarassObjective(List<SimUnit> squad, SimBuilding hall, Vector2 suspectedBase)
	{
		...
		var basePosition = _aiKnowledge.LastKnownPlayerBase ?? suspectedBase;
		var baseTile = _aiKnowledge.LastKnownPlayerBaseTile ?? Map.WorldToTile(basePosition);
		...
		foreach (var remembered in _aiKnowledge.KnownUnits)
		{
	...
	{
		return _aiKnowledge.CountKnownWorkersNear(position, radius);
	}

	private int CountKnownCombatUnitsNear(Vector2 position, float radius)
	{
		return _aiKnowledge.CountKnownCombatUnitsNear(position, radius);
	}

	private bool HasKnownTowerNear(Vector2 position, float radius)
	{
		return _aiKnowledge.HasKnownTowerNear(position, radius);
	}

	private bool HasKnownOuterTargetNear(Vector2 position, float radius)
	{
		return _aiKnowledge.HasKnownOuterTargetNear(position, radius);
	}
	...
	{
		return _aiKnowledge.EstimateKnownThreatAt(position, radius);
	}
