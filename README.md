...
- Поверх pathfinding работает ослабленный anti-stuck:
  ...
  - дальше делается мягкий local-avoidance + repath
...
- `Push` и `Harass` больше не должны слепо тоннелить в `TownHall` в самом начале захода, если есть более естественные цели или точка входа в базу
